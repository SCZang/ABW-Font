...
from models.my_generator import Generator
...
from train.train_cf_new_cb import trainGANCF
...
parser.add_argument('--original_centers', default='', type=str)
parser.add_argument('--original_sigma', default='', type=str)
parser.add_argument('--original_pc', default='', type=str)
...
def main():
    ...
    if args.content_fusion:
        # args.base_idxs = np.load(args.base_idxs)args.base_idxs_save
        # args.base_idxs_save =  args.base_idxs
        # args.base_ws_save = args.base_ws
    ...
    if local_rank == 0:
        ...
        logger = SummaryWriter(args.event_dir) if not args.on_oss else SummaryWriter(log_dir=SUMMARY_DIR, max_queue=30,
                                                                                     flush_secs=60)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if local_rank == 0:
            ...
            #if epoch == 0 or (epoch+1)%5 == 0 or epoch<=185:
        ...
        if args.content_fusion:
            ...
            if epoch == args.start_epoch:
                tmp_train_basis_loader =train_basis_loader
                new_base_weights = args.base_ws.clone()
                new_centers = torch.load(args.original_centers)
                new_sigma = torch.load(args.original_sigma)
                new_pc = torch.load(args.original_pc)

            # if epoch < 0:
            #     _,_ = trainFunc(train_loader, train_basis_loader, networks, opts, epoch, new_base_weights, args, {'logger': logger}, \
            #               detach=args.detach, quantize=args.quantize, style_con=args.style_con, \
            #               reconstruction_losses=args.recon_losses, abl=args.abl, \
            #               oss_client=oss_client if args.on_oss else None)
            # else:
            not_start_tmp_loader, temp_ws, temp_centers, temp_sigma, temp_pc = trainFunc(train_loader, tmp_train_basis_loader, networks, opts, epoch, new_base_weights, \
                                                      new_centers, new_sigma, new_pc, args, {'logger': logger}, \
                                                      detach=args.detach, quantize=args.quantize, style_con=args.style_con, \
                                                      reconstruction_losses=args.recon_losses, abl=args.abl, \
                                                      oss_client=oss_client if args.on_oss else None)
            tmp_train_basis_loader = not_start_tmp_loader
            new_base_weights = temp_ws
            new_centers =temp_centers
            new_sigma = temp_sigma
            new_pc = temp_pc
...
def build_model(args):
    ...
    if 'G' in args.to_train:
        networks['G'] = Generator(args.img_size, args.sty_dim, use_sn=False, mute=(local_rank == 0),
                                  baseline_idx=args.baseline_idx)
        networks['G_EMA'] = Generator(args.img_size, args.sty_dim, use_sn=False, mute=(local_rank == 0),
                                      baseline_idx=args.baseline_idx)

    for name, net in networks.items():
        net_tmp = net.cuda()
        if args.distributed:
            networks[name] = torch.nn.parallel.DistributedDataParallel(net_tmp, device_ids=[local_rank],
