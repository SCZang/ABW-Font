...
from models.my_generator import Generator as Generator
...
args.bs_per_font = 40
args.mini_batch = args.val_num = 16
args.local_rank = 0

print(args.data_path, args.font_len)
    ...
    with torch.no_grad():
        for s_id_now in refs_bar:  # [1, basis]
            ...
            for idx, (cnt_idx) in enumerate(trange((int)(np.ceil(chars_num / args.mini_batch)), leave=False)):
                ...
                x_basis_now = basis_each_cls[:, idx_min_now:idx_max_now].cuda(
                    non_blocking=True)  # [10, val_batch_now, 3, h, w]
                ...
                c_src_basis = torch.stack(c_src_basis_list)  # 将一个包含多个张量的列表（c_src_basis_list）沿着一个新的维度进行堆叠，形成一个新的张量。
            ...
            # logits_i = torch.mean(torch.stack(ws_i), 0)
            # # 例如，假设拉普拉斯分布的参数为 loc 和 scale
            # laplace_dist = torch.distributions.Laplace(loc=0, scale=0.005)  # 设置合适的 loc 和 scale
            # log_prior = laplace_dist.log_prob(logits_i)
            # ws_prob = torch.exp(log_prior)
            # ws_prob = ws_prob/torch.sum(ws_prob)

            logits_i = weight_now  # (10,2048)
            # logits_i = torch.sum(logits_i,1)
            laplace_dist = torch.distributions.Laplace(loc=0, scale=0.0005)
            log_probs = laplace_dist.log_prob(logits_i)
            probs = torch.exp(log_probs)
            sum_probs = torch.sum(probs, 1)
            # sum_probs = probs
            norm_probs = sum_probs / torch.sum(sum_probs)
            print(norm_probs)
            ws.append(norm_probs)
        ws = torch.stack(ws).cpu()
        torch.set_printoptions(threshold=5000)  # 设置打印阈值
        print(ws.shape)
        print(ws)
...
def get_logits_dis(tgt_feature, base_feature, opt='laplace_new'):
    assert opt in ['inner', 'l1', 'l2', 'laplace', 'laplace_new']
    # scale_tensor = torch.tensor(scale)
    base_n = base_feature.shape[0]
    batch_size = base_feature.shape[1]
    if opt == 'inner':
        w_inner = tgt_feature * base_feature
        w_mean = torch.mean(w_inner.reshape(base_n, -1), 1)
    elif opt == 'l1':
        print(tgt_feature.shape)
        print(base_feature.shape)
        w_l1 = tgt_feature - base_feature  # [10,mini_batch_size,256,20,20]
        print(w_l1.shape)
        w_mean = -torch.mean(w_l1.abs().reshape(base_n, -1), 1)
        print(w_mean.shape)
    elif opt == 'l2':
        w_l2 = (tgt_feature - base_feature) ** 2
        ...
        print(tgt_feature.shape)
        print(base_feature.shape)
        dis = tgt_feature - base_feature
        w_l1 = dis.abs().reshape(base_n, batch_size, -1)
        w_mean = torch.mean(w_l1, 2)
        print(f'w_mean{w_mean.shape}')
    elif opt == 'laplace_new':
        # w_l1 = tgt_feature - base_feature  # [10,mini_batch_size,256,20,20]
        # print(w_l1.shape)
        # w_mean = torch.mean(w_l1.abs().reshape(base_n, -1), 1)
        print(tgt_feature.shape)
        print(base_feature.shape)
        dis = tgt_feature - base_feature#(10,16,128)
        w_mean = torch.mean(dis.abs(), 2)
        print(f'w_mean{w_mean.shape}')
...
def build_model(args):
    ...
    if 'G' in args.to_train:
        ...
        networks['G_EMA'] = Generator(args.img_size, args.sty_dim, use_sn=False, mute=True,
