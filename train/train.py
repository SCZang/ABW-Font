...
def trainGAN(data_loader, networks, opts, epoch, args, additional, \
    ...
    for i in t_train:
        ...
        g_loss = args.w_adv * g_adv + args.w_rec * g_imgrec + 0.05 * args.w_rec * g_conrec + args.w_off * offset_loss
        ...
        with torch.no_grad():
            ...
            if (i + 1) % args.log_step == 0 and (
                    args.gpu == 0 or args.gpu == '0') and logger is not None and args.local_rank == 0:
                ...
                print('Epoch: [{}/{}] [{}/{}] MODE[{}] Avg Loss: D[{d_losses.avg:.2f}] G[{g_losses.avg:.2f}] '.format(
                    epoch + 1, args.epochs, i + 1, args.iters,
