...
class ImageFolerRemap(DatasetFolder):
    def __init__(self, root, transform=None, target_transform=None,
        ...
        super(ImageFolerRemap, self).__init__(root, loader, IMG_EXTENSIONS, transform=transform,
...
class ImageFolerRemapUnpairCF(DatasetFolder):
    def __init__(self, root, base_idxs, base_ws, transform=None, target_transform=None,
                 loader=default_loader, remap_table=None, with_idx=False, top_n=-1):
        super(ImageFolerRemapUnpairCF, self).__init__(root, loader, IMG_EXTENSIONS, transform=transform,
                                                      target_transform=target_transform)
    ...
    #     num_with_extension = os.path.basename(img_path)
    #     num, _ = os.path.splitext(num_with_extension)
    #
    #     # 遍历 base_idxs，加载对应的基础图像
    #     # basis = torch.stack([self._load_basis_image(idx, num) for idx in self.base_idxs])
    #     # print(font_ids)
    #     base_w = self.base_ws[target].clone()
    #
    #     if self.with_idx:
    #         return sample, index, target, img_path, basis, base_w  # image sample_index class_index
    #     return sample, target, img_path, basis, base_w  # image class_index
    def __getitem__(self, index):
        ...
        #base_ws = self.base_ws[target].clone()

        if self.with_idx:
            return sample, index, target
        return sample, target
    def _load_basis_image(self, idx, num):
        # 构建基础图像的路径
        basis_path = os.path.join(self.root, f'id_{idx}', f'{num}.png')
        basis_sample = self.loader(basis_path)

        if self.transform is not None:
            basis_sample = self.transform(basis_sample)
...
class ImageFolerRemapPair(DatasetFolder):
    def __init__(self, root, transform=None, target_transform=None,
        ...
        super(ImageFolerRemapPair, self).__init__(root, loader, IMG_EXTENSIONS, transform=transform,
                                                  target_transform=target_transform)
        self.imgs = self.samples
        self.class_table = remap_table
...
#     def __init__(self, root, base_idxs, base_ws, transform=None, target_transform=None, loader=default_loader,
#                  remap_table=None, sample_skip_base=False):
#         super(ImageFolerRemapPairbasis, self).__init__(root, loader, IMG_EXTENSIONS, transform=transform,
#                                                        target_transform=target_transform)
#         self.imgs = self.samples
#         self.class_table = remap_table
#         self.base_idxs = base_idxs
#         self.sample_skip_base = sample_skip_base
#         self.font_idxs = [s[1] for s in self.samples]  #字体label
#         self.font_N = np.max(self.font_idxs) + 1  #一共有多少字体
#         self._get_char_idx()
#         self._find_samples_from_charidx()
#
#     def _get_char_idx(self):
#         fonts_max = [0 for _ in range(self.font_N + 1)] #400个0
#         print(fonts_max)
#         char_idxs = []
#         for _, fi in self.samples:
#             char_idxs.append(fonts_max[fi])
#             fonts_max[fi] += 1
#         self.char_idxs = char_idxs #400个0-799 (320000,)
#         print(np.array(char_idxs).shape)
#         self.char_idxs_max = np.max(self.char_idxs) #值为799
#         print(self.char_idxs_max)
#         print(np.array(self.class_table).shape)
#
#     def _find_samples_from_charidx(self):
#         charidx2idx = [[] for _ in range(self.char_idxs_max + 1)]
#         charidx2idx_basis = [[] for _ in range(self.char_idxs_max + 1)]#二维数组，800个空数组
#         print(np.array(charidx2idx_basis).shape)
...
#             charidx2idx[charidx].append(i)
#         self.charidx2idx = charidx2idx
#         self.charidx2idx_basis = charidx2idx_basis#应该是(800,10)
#         print(np.array(charidx2idx).shape) #(800,400)
#         print(np.array(charidx2idx_basis)) #(800,basis_num)
#
#     def _get_sample_from_idx(self, idx):
#         path, target = self.samples[idx]
#         sample = self.loader(path)
...
#             print(base_idx)
#             base, target, path = self._get_sample_from_idx(self.charidx2idx_basis[index][base_idx])
#             assert target == self.font_idxs[self.charidx2idx_basis[index][base_idx]]
#             basis.append(base)
#             paths.append(path)  # 将路径添加到列表
#         return torch.stack(basis),paths
#
#     def __len__(self):
...
class ImageFolerRemapPairbasis(DatasetFolder):
    ...
    def _get_basis_idx(self):
        first_occurrence = {}
        for i, charidx in enumerate(self.char_idxs):
            font_idx = self.samples[i][1]
            if len(first_occurrence) < len(self.base_idxs):  # 只处理直到找到b中所有元素的位置
                if font_idx in (self.base_idxs) and font_idx not in first_occurrence:
                    first_occurrence[font_idx] = i
        #assert len(first_occurrence) == len(self.base_idxs)
        char_idx_first = [first_occurrence[element] for element in self.base_idxs]
        basis_idxs = [[] for _ in range(10)]
        for i,basis_font_id in enumerate(self.base_idxs):
            char_idxs = list(range(char_idx_first[i],char_idx_first[i] + 800))
            basis_idxs[i] = char_idxs
        self.basis_idxs = basis_idxs
    ...
    def __getitem__(self, index):  # char i
        ...
        paths = []  # 新增路径列表
        for base_idx in range(len(self.base_idxs)):
            base, target, path = self._get_sample_from_idx(self.basis_idxs[base_idx][index])
            assert target == self.font_idxs[self.basis_idxs[base_idx][index]]
            basis.append(base)
            paths.append(path)  # 将路径添加到列表
        return torch.stack(basis),paths
...
class ImageFolerRemapPairCF(DatasetFolder):
    def __init__(self, root, base_idxs, base_ws, transform=None, target_transform=None, loader=default_loader,
                 remap_table=None, sample_skip_base=False, sample_N=10, keep_unpair=True, top_n=-1):
        super(ImageFolerRemapPairCF, self).__init__(root, loader, IMG_EXTENSIONS, transform=transform,
                                                    target_transform=target_transform)
        self.imgs = self.samples
        self.class_table = remap_table
        self.base_idxs = base_idxs
...
if __name__ == '__main__':
    ...
    dataset = ImageFolerRemapPairCF('data', sample_N=20, base_idxs=list(range(10)), transform=transform,
