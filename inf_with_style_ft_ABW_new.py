...
def main():
    ...
    basis_ws = basis_ws.type(torch.float)
...
def inf(dataset, networks, opts, epoch, args, basis_ws):
    ...
    with torch.no_grad():
        ...
        style_dl = torch.utils.data.DataLoader(style_val_dataset, batch_size=args.sty_batch_size,
                                               sampler=style_sampler, shuffle=False, num_workers=8, pin_memory=True,
                                               drop_last=False)
        # [sample, path, idx]
        print(len(style_dl))
    ...
    with torch.no_grad():
        ...
        for samples, paths, _ in tqdm(content_dl) if local_rank == 0 else content_dl:
            ...
            for mb_i in trange(args.font_len // minibatch, leave=False):
                ...
                # pdb.set_trace()
                c_src = torch.einsum('bn,nc->bc', basis_ws_mb, c_src_basis)
...
def ft_style_all(s_refs, ft_dataset, G_EMA, args, basis_ws):
    ...
    for ep in ep_bar:
        ...
        for step, ((imgs_gt_, idxs_), (base_imgs_gt_, base_idxs_)) in enumerate(
                tqdm(style_dl, leave=False)) if local_rank == 0 else enumerate(style_dl):
